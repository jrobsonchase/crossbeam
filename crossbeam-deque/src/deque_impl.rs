...
use epoch::{Atomic, LocalHandle, Owned};
...
impl<T> Worker<T> {
    ...
    #[cold]
    unsafe fn resize(&self, new_cap: usize, handle: &LocalHandle) {
        // Load the back index, front index, and buffer.
    ...
    /// buffer.
    fn reserve(&self, reserve_cap: usize, handle: &LocalHandle) {
        if reserve_cap > 0 {
    ...
    /// w.push(2);
    /// ```
    pub fn push(&self, task: T, handle: &LocalHandle) {
        // Load the back index, front index, and buffer.
        let b = self.inner.back.load(Ordering::Relaxed);
        ...
        if len >= buffer.cap as isize {
            ...
            unsafe {
                self.resize(2 * buffer.cap, handle);
            }
    ...
    /// ```
    pub fn pop(&self, handle: &LocalHandle) -> Option<T> {
            ...
            Flavor::Fifo => {
                ...
                unsafe {
                    ...
                    if buffer.cap > MIN_CAP && len <= buffer.cap as isize / 4 {
                        self.resize(buffer.cap / 2, handle);
                    }
...
impl<T> Stealer<T> {
    ...
    /// assert_eq!(s.steal(), Steal::Success(2));
    /// ```
    pub fn steal(&self, handle: &LocalHandle) -> Steal<T> {
        // Load the front index.
        let f = self.inner.front.load(Ordering::Acquire);
    ...
    /// ```
    pub fn steal_batch(&self, dest: &Worker<T>, handle: &LocalHandle) -> Steal<()> {
        // Load the front index.
        ...
        dest.reserve(batch_size, handle);
    ...
    /// assert_eq!(w2.pop(), Some(2));
    /// ```
    pub fn steal_batch_and_pop(&self, dest: &Worker<T>, handle: &LocalHandle) -> Steal<T> {
        // Load the front index.
        let mut f = self.inner.front.load(Ordering::Acquire);
